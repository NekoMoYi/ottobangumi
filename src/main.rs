...
async fn main() -> Result<(), BoxErr> {
    ...
    let rss_interval: u32 = env::var("RSS_INTERVAL").unwrap_or("3600".to_string()).parse().unwrap();
    let db_path = env::var("DATABASE").unwrap_or("otto.db".to_string());
    let db = Arc::new(database::Client::new(&db_path)?);
    let bot_config = Arc::new(bot::BotConfig {
        user_ids: env::var("USER_IDS").unwrap_or_default().split(',').map(|id| id.parse().unwrap()).collect(),
        proxy: proxy,
        not_contains: env::var("NOT_CONTAINS").unwrap_or_default().split(',').map(|s| s.to_string()).collect(),
        tmp_dir: env::var("TMP_DIR").unwrap_or("tmp".to_string()),
        lib_dir: env::var("LIB_DIR").unwrap(),
    });
    let bot = bot::MyBot::new(bot_config.clone(), db.clone()).await?;
            ...
            match bangumis {
                Ok(bangumis) => {
                    ...
                    for b in bangumis {
                        ...
                        if let Err(e) = update_rss(b, db.clone(), bot_config.clone(), tg_bot.clone()).await {
...
async fn update_rss(b: database::Bangumi, db: Arc<database::Client>, cfg: Arc<bot::BotConfig>, tg: Arc<teloxide::Bot>) -> Result<(), BoxErr> {
    let rss = mikan::MikanRss::from_url(&b.rss_url).set_proxy(cfg.proxy.clone())?.fetch().await?;
    for ep in rss.items.iter().rev() {
        if cfg.not_contains.iter().any(|s| ep.title.contains(s)) || ep.torrent_hash.is_empty() || b.downloaded.contains(&ep.torrent_hash) {
            continue;
        }
        ...
        downloader.download_by_torrent_to(&torrent_path, &torrent_hash, &save_dir, &save_name).await?;
        db.add_downloaded(b.id, &torrent_hash)?;
        for chat_id in cfg.user_ids.iter() {
            tg.send_message(teloxide::types::ChatId(*chat_id as i64), format!("{} updated.", save_name)).await?;
        }
    }
    Ok(())
}